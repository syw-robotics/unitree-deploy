...
### Gate 控制 (`policy.yaml` 的 `gate_control`)

- `mode: policy`: 加载 `policy_path`，由 actor 内部的 log variance 和 gate net
  计算并平滑 gate。
- `mode: manual`: 加载 `manual_path`，直接把 `value` 作为 gate 输入。该 ONNX
  不包含 log variance 和 gate net，因此这两个网络不会运行。
- `value`: manual 模式的固定 gate，范围 `[0, 1]`。`0` 表示屏蔽外感 latent，
  `1` 表示完整使用外感 latent。
